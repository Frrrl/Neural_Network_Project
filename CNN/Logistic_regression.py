...
#label = list(label)  # 抽取出标签
model = FNN_model()
opt = tf.keras.optimizers.SGD(learning_rate=0.01)  # 优化器的设定，采用SGD优化器，学习率为0.01
for i in range(200):
    loss, accuracy, w, b = train_one_step(model, opt, data, label)
    if i % 10 == 0:
        print("step:", i, "loss:", loss.numpy(), "accuracy: ", accuracy.numpy(),"w1:", w[0].numpy(), "w2 :", w[1].numpy(), "b:",b.numpy())
# </editor-fold>
